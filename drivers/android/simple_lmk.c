...
static int simple_lmk_init_set(const char *val, const struct kernel_param *kp)
{
	...
	if (!atomic_cmpxchg(&init_done, 0, 1)) {
		thread = kthread_run_perf_critical(cpu_perf_mask, simple_lmk_reaper_thread, NULL,
				     "simple_lmkd_reaper");
		BUG_ON(IS_ERR(thread));
		thread = kthread_run_perf_critical(cpu_perf_mask, simple_lmk_reclaim_thread, NULL,
				     "simple_lmkd");
		BUG_ON(IS_ERR(thread));
